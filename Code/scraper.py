    ...
    for row in rows:
        ...
        try:
            tourn_model = TournamentModel(
                tourn_id=tourn_id, url=url, dates=dates, name=name, season=season, category=category
            )
...
def parse_matches(html: str, tourn_id: int) -> List[List[Any]]:
    ...
    for match in matches:
        match_id_str = match.get("data-match-id")
        if not match_id_str:
            continue
        ...
        if p1_div:
            ...
            p1_a = p1_div.find("a")
            player_1_url = p1_a["href"].rsplit("/", 2)[0] if p1_a else ""
        else:
        ...
        if p2_div:
            ...
            p2_a = p2_div.find("a")
            player_2_url = p2_a["href"].rsplit("/", 2)[0] if p2_a else ""
        else:
